(function() {
    ...
    var version_regexs = ['(?:\\d)', '(?:\\d\\.\\d[\\w\\d\\.]*)', '(?:dev)', '(?:release/\\d.\\d[\\x\\d\\.]*)'];
    ...
    function build_language_select(current_language) {
        ...
        $.each(all_languages, function(language, title) {
            if (language == current_language)
                buf.push('<option value="' + language + '" selected="selected">' + all_languages[current_language] + '</option>');
            else
                buf.push('<option value="' + language + '">' + title + '</option>');
        });
        if (!(current_language in all_languages)) {
            // In case we're browsing a language that is not yet in all_languages.
            buf.push('<option value="' + current_language + '" selected="selected">' + current_language + '</option>');
    ...
    function on_version_switch() {
        ...
        var new_url = url.replace('/' + current_language + current_version, '/' + current_language + selected_version);
        if (new_url != url) {
            navigate_to_first_existing([new_url, url.replace('/' + current_language + current_version, '/' + selected_version), '/' + current_language + selected_version, '/' + selected_version, '/']);
        }
    }
    ...
    function on_language_switch() {
        ...
        if (selected_language == 'en/')
            ...
            selected_language = '';
        var new_url = url.replace('/' + current_language + current_version, '/' + selected_language + current_version);
        if (new_url != url) {
...
<a href="/" id="indexlink">Documentation</a> &#187;';
        ...
        var probable_places = ["body>div.related>ul>li:not(.right):contains('Documentation'):first", "body>div.related>ul>li:not(.right):contains('documentation'):first", ];
